...
$html=  '</table>
  <div id="contenedor" style="display:flex;height:415px; overflow: scroll;">
   <div id="segundo" style="width: 74%; float:left;">';
	...
	$sql="SELECT `idcuentaspromotor`, `cue_numeroguia`,'ENTREGA' as estado,cue_tipoevento, cue_idservicio,cue_valorflete,cue_kilostotal,cue_transferencia,inner_sedes
	FROM cuentaspromotor inner join ciudades on idciudades=cue_idciudadori  where  cue_estado in (10) and  cue_fecha>='$param2' and cue_fecha<='$param3 23:59:59' and cue_idciudaddes in $ciudades 
	...
	$DB->Execute($sql); $va=0; 
	 ...
	 while($rw1=mysqli_fetch_row($DB->Consulta_ID))
	 {
		 ...
		 $estado=$rw1[2];
		 if($estado=='Contado' and $rw1[9]!='Efectivo' and $rw1[9]!=''){
			$estado=$rw1[9];
		 }

		 $tipopago1=$tipopago[$rw1[3]];
		 $kilos=$rw1[6];
		 ...
		 if($kilos=='0'){
			//select gui_tiposervicio from guias
			 $sql1="SELECT (ser_peso+ser_volumen) as kilos from servicios where ser_guiare='$rw1[1]'";
			$DB1->Execute($sql1);
		 ...
		 $valorporcentaje=0;
		 if($rw1[5]>0){
			 $sql6="SELECT `idporcentajespaquetes`, `por_porcentaje`,por_porcentajeempresa FROM `porcentajespaquetes`  WHERE  por_idsede='$param1'  and por_idsededestino='$rw1[8]' and por_tiposervicio='$tipopago1' and '$kilos'>=por_kilosgramosmin and '$kilos'<=por_kilogramosmaximo and (por_idpaquete='$rw6[0]' or por_idpaquete='') order by idporcentajespaquetes desc limit 1";
			 $DB1->Execute($sql6);

			 $rw3=mysqli_fetch_row($DB1->Consulta_ID);
			 $porcentaje=$rw3[1];
			 $porempresa=$rw3[2];
		     $valorporcentaje=($rw1[5]*$porcentaje)/100;
		     $valorporempresa=($rw1[5]*$porempresa)/100;
		 }
	...
	/* 	 $sql2="UPDATE `cuentaspromotor` SET cue_porcentaje='$porcentaje',cue_porempresa='$porempresa',cue_valorporcantaje='$valorporcentaje',cue_valorporempresa='$valorporempresa' where cue_idservicio=$rw1[4]";
		 $DB1->Execute($sql2);	 */
	...
	FROM cuentaspromotor  inner join ciudades on idciudades=cue_idciudadori  where cue_estado in (4,6,14,7,8,9,10)  and cue_fecharecogida>='$param2' and cue_fecharecogida<='$param3 23:59:59' and cue_idciudadori in $ciudades and cue_tipoevento=1
	";

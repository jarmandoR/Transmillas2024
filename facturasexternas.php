...
$FB->titulo_azul1("Facturas",9,0,5);  
...
<?php 
	...
	//echo $fechainicial;
	$FB->llena_texto("Fecha de Inicial:", 1, 10, $DB, "", "", "$fechainicio", 1, 0);
	$FB->llena_texto("Fecha de Final:", 2, 10, $DB, "", "", "$fechaactual", 4, 0);
	$FB->llena_texto("Cliente:",3, 2, $DB, "(SELECT `cre_nombre`,`cre_nombre` FROM `creditos`)", "", "$param9",17,1);
	$FB->llena_texto("Estado Facturas:",4,82,$DB,$estadofac,"","",4,0);
	$FB->llena_texto("Valor Mayor a:", 5, 1, $DB, "", "","$param2", 4,0);
	$FB->llena_texto("# Factura:", 6, 1, $DB, "", "","$param2", 4,0);
...
echo "<td><button type='button' class='btn btn-success' onclick='crearfaactura();'>Crear Factura </button></td>";
echo "<td><button type='button' class='btn btn-success' onclick='llena_datos(3, $nivel_acceso, \"id_nombre\", \"ASC\");'>Consultar Factura</button></td></tr>";

...
$sedes="SELECT sed_mostrarTelefonoClientes FROM `sedes` WHERE idsedes='$id_sedes' ";
$DB1->Execute($sedes); 
$mostrarNumero=$DB1->recogedato(0);
...
if($param31!="" and $param36!=""){ 
 $conde1="and $param31 like '%$param36%' "; 
  ...
  $sql="SELECT `idservicios`,`ser_fechaentrega`,`cli_nombre`,`cli_telefono`,`cli_direccion`, `ser_destinatario`, `ser_telefonocontacto`,`ser_direccioncontacto`,`ciu_nombre`,`ser_prioridad`,ser_estado,ser_visto,usu_nombre,`ser_fechaasignacion`,ser_valorprestamo,ser_llamadarecogido,cli_idciudad,ser_ciudadentrega
...
while($rw1=mysqli_fetch_row($DB->Consulta_ID))
{
	...
	$p=$va%2;
		...
		if ($mostrarNumero=="si" or $mostrarNumero=="SI" or $mostrarNumero=="Si") {
			$telefono=$rw1[3];
			$telefono1=$rw1[6];
		}else {
			$telefono=" ";
		...
		if($rw1[10]==4) {	
			...
			echo "<td align='center'  data-toggle='tooltip' data-placement='top'  title='' ><a  onclick='pop_dis1($id_p, \"Verificar Datos\", \"recogerpaquete.php\");setTimeout(function(){buscarservicio($rw1[16],$rw1[17],\"$creditos\",\"$rw1[9]\",$valortserv)},1000);'  style='cursor: pointer;' title='Verificar Datos' >

...
function llena_datos(ex, nivel, ordby, asc)
{
	...
	if(ex==1){
		...
		destino="detalle_programartareas.php?param32="+p1+"&param33="+p2+"&param35="+p3+"&param36="+p4;
...
$FB->titulo_azul1("Ver tareas programadas Diarias",9,0,5);  
...
$FB->llena_texto("Estado:",36,82, $DB, $estadotarea, "", "", 1, 0);

if($nivel_acceso==1 or $nivel_acceso==12){
	$FB->llena_texto("Sede Origen:",35,2,$DB,"(SELECT `idsedes`,`sed_nombre` FROM sedes where idsedes>0 )", "cambio_ajax2(param33.value, 36, \"llega_sub1\", \"param32\", 1, this.value)", "$id_sedes", 4, 0);

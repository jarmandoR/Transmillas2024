...
/* 			echo "<td align='center'  data-toggle='tooltip' data-placement='top'  title='' ><a  onclick='generarcodigo($id_p)';  target='_blank'  style='cursor: pointer;' title='Imprimir Codigo' >
		...
		$sql="SELECT `idservicios`,`ser_fechaentrega`,`cli_nombre`, `cli_telefono`,`cli_direccion`, `ser_destinatario`, `ser_telefonocontacto`,`ser_direccioncontacto`,`ciu_nombre`,`ser_prioridad`,ser_fecharegistro,ser_consecutivo,ser_guiare,cli_idciudad,ser_estado,'1' as tipoc,ser_clasificacion	FROM serviciosdia where $conde1  
		union 
	...
	$orden=$DB->recogedato(0); */
		...
		$preop=$DB->recogedato(0);
		 ...
		 $idestadoguia="SELECT  CONCAT(seg_estado,'|',seg_direccion,'|',seg_tipo,'|',seg_idservicio)  as id FROM seguimientoruta where `seg_idusuario`='$id_usuario' and seg_fecha like '%$fechaactual%' and seg_estado!='Cambioruta' order by `seg_fechaestado` desc limit 1";
